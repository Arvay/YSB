...
gulp.task('revCss', function(){
    ...
    return gulp.src(cssSrc)
        .pipe(rev())
        .pipe(minifyCss())
        .pipe(gulp.dest('../../prd/static/css/'))
        .pipe(rev.manifest())
        .pipe(gulp.dest('../../prd/static/rev/css/'));
});
...
gulp.task('revJs', function(){
    ...
    return gulp.src(jsSrc)
        ...
        .pipe(uglify())
        .pipe(gulp.dest('../../prd/static/js/'))
        .pipe(rev.manifest())                       //生成rev-mainfest.json文件作为记录
        .pipe(gulp.dest('../../prd/static/rev/js/'));
});
...
gulp.task('revHtml', function () {
  console.log('正在------------------------------------------------------>替换 html路径');
    return gulp.src(['../../prd/static/rev/**/*.json', '../templates/*.html'])
        .pipe(revCollector())                         //替换html中对应的记录
        .pipe(gulp.dest('../../prd/templates/'));                     //输出到该文件夹中
});
//开发构建
